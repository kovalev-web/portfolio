...
void main() {
  ...
  vec2 q = vec2(
    fbm(p + vec2(1.2, 0.0) + t),
    fbm(p + vec2(5.2, 1.3) - t * 0.6)
  );

  vec2 r = vec2(
    fbm(p + 2.5 * q + vec2(1.7, 9.2) + t * 0.35),
    fbm(p + 2.5 * q + vec2(8.3, 2.8) - t * 0.25)
  );

  float f = fbm(p + 3.0 * r);

  vec3 col = mix(uColorA, uColorB, clamp(f * 2.0, 0.0, 1.0));
  col = mix(col, uColorC, clamp(length(q) * 0.7, 0.0, 1.0));
  col = mix(col, uColorD, clamp(r.x * 0.8, 0.0, 1.0));
